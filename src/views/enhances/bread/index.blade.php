        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="voyager-trash"></i><span id="msg_modal_delete">{!! __('voyager::bread.delete_bread_quest', ['table' => '<span id="delete_builder_name"></span>']) !!}</span></h4>
                </div>
    ...
    <script>
        ...
        $(function () {
            ...
            $('table .actions').on('click', '.delete', function (e) {
                id = $(this).data('id');
                name = $(this).data('name');
                txt.text( txt.text().replace('API', 'BREAD') );
                $('#delete_builder_name').text(name);
                $('#delete_builder_form')[0].action += '/' + id;
                $('#delete_builder_modal').modal('show');
            });

            $('table .actions').on('click', '.bread_delete', function (e) {
                id = $(this).data('id');
                name = $(this).data('name');
                txt.text( txt.text().replace('BREAD', 'API') );
                $('#delete_builder_name').text(name);
                $('#delete_builder_form')[0].action =  "{{ route('fibonacci.database.api.delete', ['id' => null]) }}" + '/' + id;
                $('#delete_builder_modal').modal('show');
